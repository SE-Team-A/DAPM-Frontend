...
const takeSnapshot = (state: PipelineState) => {
  var activePipeline = state.pipelines.find(
    (pipeline) => pipeline.id === state.activePipelineId
  );
  if (!activePipeline) return;
  activePipeline?.history?.past?.push({
    nodes: activePipeline.pipeline.nodes,
    edges: activePipeline.pipeline.edges,
  });
};
...
const pipelineSlice = createSlice({
  ...
  reducers: {
    addNewPipeline: (
      state,
      { payload }: PayloadAction<{ id: string; flowData: NodeState }>
    ) => {
      state.pipelines.push({
        id: payload.id,
        name: "unnamed pipeline",
        pipeline: payload.flowData,
        history: { past: [], future: [] },
        imgData: "",
      } as PipelineData);
    ...
    setImageData: (
      state,
      { payload }: PayloadAction<{ id: string; imgData: string }>
    ) => {
      var pipeline = state.pipelines.find(
        (pipeline) => pipeline.id === payload.id
      );
      if (!pipeline) return;
    ...
    undo(state) {
      ...
      activePipeline.history.future.push({
        nodes: activePipeline.pipeline.nodes,
        edges: activePipeline.pipeline.edges,
      });
    ...
    addHandle: (state, { payload }: PayloadAction<string>) => {
      var activeFlowData = state.pipelines.find(
        (pipeline) => pipeline.id === state.activePipelineId
      )?.pipeline;
      activeFlowData?.nodes
        .find((node) => node.id === payload)
        ?.data?.templateData?.sourceHandles.push({ type: "source", id: "1" });
    },
    updateSourceHandle: (
      state,
      {
        payload,
      }: PayloadAction<{ nodeId?: string; handleId?: string; newType?: string }>
    ) => {
      ...
      const targetNode = activeFlowData.nodes.find(
        (node) => node.id === nodeId
      );
      ...
      const handleToUpdate = targetNode.data.templateData.sourceHandles.find(
    ...
    updateTargetHandle: (
      ...
      // Find the handle to update within the sourceHandles
      const handleToUpdate = targetNode.data.templateData.targetHandles.find(
        (handle) => handle.id === handleId
      );
    ...
    updateNode: (
      state,
      { payload }: PayloadAction<Node<NodeData> | undefined>
    ) => {
      if (!payload) return;
      var activeFlowData = state.pipelines.find(
        (pipeline) => pipeline.id === state.activePipelineId
      ...
      const index = activeFlowData?.nodes.findIndex(
    ...
    removeNode: (state, { payload }: PayloadAction<Node<NodeData>>) => {
      ...
      activeFlowData.nodes = activeFlowData.nodes.filter(
        (node) => node.id !== payload.id && node.parentNode !== payload.id
      );
      activeFlowData.edges = activeFlowData.edges.filter(
        (edge) =>
          !payload.data?.templateData?.sourceHandles.find(
            (data) => data.id === edge.sourceHandle
          ) &&
          !payload.data?.templateData?.targetHandles.find(
            (data) => data.id === edge.targetHandle
          )
      );
    },
    ...
    updateEdge: (
      state,
      { payload }: PayloadAction<Edge<EdgeData> | undefined>
    ) => {
      if (!payload) return;
      var activeFlowData = state.pipelines.find(
        (pipeline) => pipeline.id === state.activePipelineId
      ...
      if (!activeFlowData) return;
      const index = activeFlowData.edges.findIndex(
        (edge) => edge.id === payload.id
      );
      const strokeColor =
        payload.data?.filename === undefined ||
        payload.data?.filename === "" ||
        payload.data?.filename === null
          ? "red"
          : "white";
    ...
    onConnect: (state, { payload }: PayloadAction<Connection>) => {
      ...
      const strokeColor =
        activeFlowData.nodes.find((node) => node.id == payload.target)?.type ===
        "dataSink"
          ? "red"
          : "white";

      activeFlowData.edges = addFlowEdge(
        { ...payload, type: "default" },
        activeFlowData.edges
      );
    },
