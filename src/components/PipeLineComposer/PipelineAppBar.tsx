...
export default function PipelineAppBar() {
      ...
      .map((edge) => {
        if (edge.data?.filename) {
          ...
          return {
            ...
            position: {
              x: originalDataSink.position.x,
              y: originalDataSink.position.y,
            },
            id: getNodeId(),
            ...
            flowData?.nodes
              ...
              .concat(
                flowData?.nodes
                  ...
                  .map((node) => node as Node<OrganizationNodeData>)
                  .map((node) => {
                    return {
                      type: node.type,
                      data: {
                        ...
                        instantiationData: {
                          ...
                          organization: {
                            //...node?.data?.instantiationData.algorithm,
                            name: node?.data?.instantiationData.organization
                              ?.name,
                            organizationId:
                              node.data.instantiationData.organization?.id,
                            apiUrl:
                              node?.data?.instantiationData.organization
                                ?.apiUrl,
                          },
                        },
                      },
  ...
  const savePipeline = async () => {
    ...
    if (pipelineID?.startsWith("pipeline")) {
      ...
      console.log(
        `Pipeline with id: ${pipelineId} has been saved successfully!`
      );
      window.location.href = process.env.REACT_APP_FRONTEND_URL + "";
    } else if (pipelineID) {
      const pipelineId = await editPipeline(
        org.id,
        repo.id,
        pipelineID,
        pipeline
      ...
      console.log(
        `Pipeline with id: ${pipelineID} has been edited successfully!`
      );
    }
  };
